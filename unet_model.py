...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            # modules.append(('conv', conv3d(512, out_channels, kernel_size, bias, padding=padding)))
        elif char == 'g':
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
                ...
                encoder = Encoder(f_maps[i - 1], out_feature_num, basic_module=basic_module, conv_layer_order=layer_order, \
                                 conv_kernel_size=conv_kernel_size, num_groups=num_groups, pool_kernel_size=pool_kernel_size, padding=conv_padding)
            encoders.append(encoder)
        self.encoders = nn.ModuleList(encoders)

        #NOTE create decoder path
        decoders = []
        ...
        for i in range(len(reversed_f_maps) - 1):
            ...
            if i == 0:
                ...
                _upsample = True
            decoder = Decoder(in_feature_num, out_feature_num, basic_module=basic_module, conv_layer_order=layer_order, \
                            conv_kernel_size=conv_kernel_size, num_groups=num_groups, padding=conv_padding, upsample=_upsample)
...
class UNet3D(Abstract3DUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=False, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=4, is_segmentation=True, testing=False, conv_padding=1, **kwargs):
